...
for cnt_bin, bin in enumerate(np.array_split(srt_idx, n)):
    ...
    for cnt, idx in enumerate(bin):
        print(
            f"Processing bin N. {cnt_bin} for spectrum {cnt} of {bin.size}! The SNR is {snrs[idx]}",
            end="\r",
        )
        # mmap_mode = 'r', reads only the part I'm interested in.
        ifu_spectra = np.load(
            f"{all_data_proc}/{plate_ifus[idx]}.npy", mmap_mode="r"
        )
    ...
    wkeep = np.where(
        np.count_nonzero(~np.isfinite(spectra), axis=1) < bin.size // 10
    )
